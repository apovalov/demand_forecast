...
def extract_features(
    ...
    # print('1111add_features')
    # print(df_features.head(50))

    # Adding targets
    add_targets(df_features, targets)
    # print('1111add_targets')
    # print(df_features.head(50))
...
def split_train_test(
    ...
    #
    # # Сохраняем df_train в временный файл
    # train_file_path = '/tmp/df_train.pkl'
    # with open(train_file_path, 'wb') as f:
    #     pickle.dump(df_train, f)
    # # Загружаем df_train как артефакт
    # task.upload_artifact(name='df_train', artifact_object=df_train)
    #
    # # Сохраняем df_test в временный файл
    # test_file_path = '/tmp/df_test.pkl'
    # with open(test_file_path, 'wb') as f:
    #     pickle.dump(df_test, f)
    # # Загружаем df_test как артефакт
    # task.upload_artifact(name='df_test', artifact_object=df_test)


    print(f"Train and test data splitted. Train shape: {df_train.shape}, Test shape: {df_test.shape}")
...
def fit_model(
    ...
    model = MultiTargetModel(features=features, horizons=horizons, quantiles=quantiles)
...
def fit_eval_model(
    ...
    eval_model = MultiTargetModel(features=features, horizons=horizons, quantiles=quantiles)
...
def evaluate(
    ...
    # Evaluate model
    losses = evaluate_model(df_true=df_test, df_pred=df_pred, quantiles=quantiles, horizons=horizons)
...
def deploy_model(
    ...
    # Using the test_losses function to perform additional quality checks
    # losses = evaluate_model(losses, df_pred)
    # if not losses:
    #     print("Model quality check failed based on test_losses evaluation.")
    #     return
    print(f"Losses: {losses}")
    # Additional quality checks if necessary
    # ...

    print("Model quality checks passed. Saving production model...")

    # Save the production model to the specified path
    with open(model_path, 'wb') as file:
        pickle.dump(model, file)

    print(f"Production model saved at {model_path}")
...
def run_pipeline(
    ...
    # Deploying the model
    deploy_model(prod_model, model_path, losses, df_pred)
